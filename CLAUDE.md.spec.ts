...
- \`.github/\` — CI workflows (\`ci.yml\` is the gate). \`.vigiles/\` — vigiles's own runtime output (gitignored: hook observations, eval locks, the runs ledger).`,
  ...
  rules: {
    ...
    "prefer-immutable-updates": guidance(
      "Build a new value, don't edit an existing one — and where the value crosses a function boundary, make the edit IMPOSSIBLE rather than merely absent. The failure this rule is written for (measured 2026-09-15 in `mergeHooksJson`): the function took `existing`, made a SHALLOW copy `{ ...(existing.hooks ?? {}) }`, and assigned into it per event. That copy shares its ARRAYS with the caller's object, and the container's type was the mutable `Record<string, HookEntry[]>` — so the function was pure only because the author happened to write `hooks[e] = [...]` instead of `hooks[e].push(...)`, and nothing distinguished the two. Purity resting on which line the next author reaches for is exactly the discipline this codebase refuses to rely on elsewhere. THREE RULES, strongest first. (1) TYPE THE CONTAINER `readonly` — `readonly T[]` / `Readonly<Record<K, readonly T[]>>` on every shape that is read by, or returned to, someone else. This is the `ts-essentials` make-illegal-states-irrepresentable move applied to mutation: `push` stops being a style question and becomes a tsc error, which is free (no dependency, no rule to tune) and cannot be forgotten. (2) UPDATE BY CONSTRUCTION — `{ ...before, ...rewritten }`, `Object.fromEntries(Object.entries(x).map(...))`, `[...kept, ...added]` — rather than an accumulator you assign into by key. A local `let`, a `for` loop, or a `reduce` writing an object literal it created ITSELF are all fine; the line is whether the thing you edit can be observed by anyone else. (3) NEVER MUTATE A PARAMETER'S PROPERTIES — that edit is visible to the caller by definition, and it is the one case a linter can decide on its own: `no-param-reassign` with `props: true` is on as a WARNING in `eslint.config.mjs`. NO LIBRARY FOR THIS, and that is measured, not a preference. `eslint-plugin-functional` on this corpus: `immutable-data` + `no-let` + `no-loop-statements` = 2309 findings across 149 of 210 files, `prefer-immutable-types` (parameters only) = 685, `type-declaration-immutability` = 413 — a rule that opens with four figures gets silenced the day it lands (`lint-rule-calibration`: severity tracks CONFIDENCE, and a warning nobody reads catches nothing). The core rule measured 18, of which 13 are real: a list of work, so it is on at `warn`, and it gates nothing until that reaches zero. Lodash is NOT the answer either — it is not an immutability library (`_.merge` mutates its first argument, which is this very bug with a nicer name), and the CLI is deliberately runtime-dep-light; a deep-update helper would be the thing to reach for only once a shape genuinely needs one, and none does today.",
    ),
