...
The test the owner uses: *can an adapter change what the domain SEES, rather than how it interprets what it sees?* Round 1 stated the rule ("takes only what the domain enumerated; returns a label; never a root") and then proposed `localAdvisories(root)`, which takes a root. That is corrected here. Per method:
