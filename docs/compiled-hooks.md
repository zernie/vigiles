...
- [The roles](#the-roles)
- [Role and event have to fit](#role-and-event-have-to-fit)
...
| **Category mistakes** — "block on a `SessionStart`/`PostToolUse` hook", whose decision is a documented no-op ([#4362](https://github.com/anthropics/claude-code/issues/4362)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Two layers, because the first is not enough alone. Each role has its own return type, so returning a `deny` from an inject is a **`tsc` type error**. But a role can be legal while the EVENT cannot support it — a prompt gate on `PreToolUse` reads a prompt the event does not carry, and typechecks fine. Measured across six such pairs: `tsc` caught one, and `vigiles compile` does not run `tsc` at all. The pair is now checked **at compile time** against the harness's capability table — see [Role and event have to fit](#role-and-event-have-to-fit). |
...
That case is not hypothetical: before this check it compiled clean, ran on every tool call, and allowed everything — an absent `prompt` reads as `""`, and every test against `""` is false. Three shapes are refused:

| Refused                                                                                                               | Why                                                    |
| --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| a role whose field the event does not carry (prompt gate on `PreToolUse`, file gate on `Stop`)                        | it decides on an absent field, so it allows everything |
| a **tool matcher** on an event that carries no tool                                                                   | the matcher can match nothing, so the hook never fires |
| a gate on an event honouring no veto **and** no feedback (`SessionStart`, `SessionEnd`, `Notification`, `PreCompact`) | the deny is discarded silently                         |

One shape is **warned** about rather than refused — a gate on `PostToolUse`. Its `deny` does reach the model, as feedback after the tool already ran, which is a legitimate nudge channel (vigiles ships a hook on it). It simply does not block, so you get a line saying so instead of an error.

A related refusal, same root: a bare `experimental_defineHook` matches `Bash` **by construction**, so passing it a `match` is now an error rather than a silently ignored field. Before, `match: tools("mcp__github__merge_pull_request")` compiled to a matcher of `Bash` — asking on every shell command while the MCP call it was written for went through unguarded.

**Where the check is silent, and why that is deliberate:** Claude Code documents 31 hook events; vigiles records the capabilities of nine — the ones a vendor doc or a measurement actually covers. On the other 22 it says nothing. Guessing would turn our gap into your compile error, which is the wrong trade: a false rejection costs more than a missed one.
...
**Multi-harness.** The typed program is harness-neutral; only the merged config differs. `vigiles compile --harness=codex` merges a Codex `config.toml` `[[hooks.<event>]]` block (an anchored-regex matcher) instead of Claude Code's `settings.json` JSON. **A repo that declares both harnesses** (`"harnesses": { "claude-code": {}, "codex": {} }` in `.vigilesrc.json`) gets the SAME hook installed into **both** configs — `vigiles compile` (no flag) fans out to every declared harness, so a hook is never silently wired on one and missing on the other. The gate runtime is shared, since Codex vetoes a tool call via `exit 2` exactly as Claude Code does. **Inject** output (`additionalContext`) is confirmed shared with Codex too, so an inject hook compiles for both — `compile --harness=codex` only warns if you target an event that harness doesn't honor for injection (each harness declares its `injectableEvents`). **React** output is the one piece still Claude-Code-confirmed only, so `compile --harness=codex` warns loudly on a react hook rather than ship a maybe-no-op.
